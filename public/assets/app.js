...
const API_URL = window.KICKS_API_URL || 'sessions.php';
const elements = {
...
function updateTimer() {
  if (activeSession) elements.timer.textContent = formatDuration(elapsedSeconds(activeSession));
}
...
function renderCounter() {
  ...
  elements.undo.disabled = saving || activeSession.movements.length === 0;
  elements.finish.disabled = saving || activeSession.movements.length === 0;
  elements.kick.disabled = saving;
  elements.note.disabled = saving;
  elements.finish.textContent = saving ? 'Guardando…' : 'Finalizar y guardar';
...
async function refreshHistory() {
  try {
    history = await loadHistory(API_URL);
    renderHistory();
  } catch (error) {
...
elements.kick.addEventListener('click', () => {
  activeSession = addMovement(activeSession);
  renderCounter();
...
elements.note.addEventListener('input', () => {
  if (activeSession) activeSession = { ...activeSession, notes: elements.note.value.slice(0, 500) };
});

elements.finish.addEventListener('click', async () => {
  if (!activeSession?.movements.length || saving) return;
  const completed = finishSession(activeSession, elements.note.value);
  saving = true;
  renderCounter();
  try {
    await saveSession(API_URL, completed);
    activeSession = null;
    await refreshHistory();
    announce('Sesión guardada en la base de datos.');
  } catch (error) {
    announce(error.message);
  } finally {
    saving = false;
